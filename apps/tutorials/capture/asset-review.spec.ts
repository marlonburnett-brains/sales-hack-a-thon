...
/**
 * Asset Review & Approval Tutorial Capture
 *
 * Captures the complete asset review workflow: compliance checks with severity
 * levels, fix and re-check cycle, reject and regeneration flow, and final
 * approval. Uses stage-aware asset-review mock route for compliance data.
 *
 * 17 steps covering: review-queue -> compliance-check -> compliance-issues ->
 * regenerating -> re-review -> approved -> reject-artifact -> regenerating ->
 * re-review -> approved
 */

const TUTORIAL_ID = "asset-review";
...
test.describe("Asset Review & Approval Tutorial Capture", () => {
  // Mutable stage ref -- updated by capture loop, read by browser-side mocks
  let currentStageRef = "review-queue";
  ...
  test("capture all steps", async ({ page }) => {
    ...
    for (let i = 0; i < script.steps.length; i++) {
      ...
      await test.step(`Step ${i + 1}: ${step.id}`, async () => {
        ...
        } else if (step.url === currentUrl) {
          // Same URL but different stage -- reload to pick up new fixture data
