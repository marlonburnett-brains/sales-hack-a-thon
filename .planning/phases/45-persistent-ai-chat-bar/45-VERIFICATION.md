...
| `apps/web/src/app/api/deals/[dealId]/chat/route.ts` | Typed web proxy for bootstrap and streamed sends using the currently validated bearer auth contract | ✓ VERIFIED | Bootstrap and streaming requests proxy with bearer auth at `apps/web/src/app/api/deals/[dealId]/chat/route.ts:13` and `apps/web/src/app/api/deals/[dealId]/chat/route.ts:102`. |
| `apps/web/src/app/api/deals/[dealId]/chat/bindings/route.ts` | Typed web proxy for binding confirmation using the currently validated bearer auth contract | ✓ VERIFIED | Binding saves proxy with bearer auth at `apps/web/src/app/api/deals/[dealId]/chat/bindings/route.ts:23` and `apps/web/src/app/api/deals/[dealId]/chat/bindings/route.ts:61`. |
| `apps/web/src/lib/api-client.ts` | Shared server-side bridge that matches the currently validated protected agent auth contract | ✓ VERIFIED | `fetchAgent()` uses bearer auth at `apps/web/src/lib/api-client.ts:26`, and bootstrap/send/binding helpers delegate through it at `apps/web/src/lib/api-client.ts:333`, `apps/web/src/lib/api-client.ts:352`, and `apps/web/src/lib/api-client.ts:365`. |
...
| `apps/web/src/app/api/deals/[dealId]/chat/route.ts` | `apps/agent/src/mastra/index.ts` | typed GET/POST proxy to protected chat routes | WIRED | The proxy currently uses bearer auth at `apps/web/src/app/api/deals/[dealId]/chat/route.ts:16` and forwards to `/deals/:dealId/chat` at `apps/web/src/app/api/deals/[dealId]/chat/route.ts:66` and `apps/web/src/app/api/deals/[dealId]/chat/route.ts:102`. |
| `apps/web/src/app/api/deals/[dealId]/chat/bindings/route.ts` | `apps/agent/src/mastra/index.ts` | binding confirmation proxy | WIRED | The binding proxy currently uses bearer auth at `apps/web/src/app/api/deals/[dealId]/chat/bindings/route.ts:26` and forwards to the protected binding route at `apps/web/src/app/api/deals/[dealId]/chat/bindings/route.ts:61`. |
| `apps/web/src/lib/api-client.ts` | `apps/agent/src/mastra/index.ts` | server actions delegate to protected agent routes | WIRED | `fetchAgent()` currently uses bearer auth at `apps/web/src/lib/api-client.ts:32`, and deal-chat helpers call the protected agent endpoints at `apps/web/src/lib/api-client.ts:347`, `apps/web/src/lib/api-client.ts:356`, and `apps/web/src/lib/api-client.ts:369`. |
...
The prior auth-contract blocker is operationally closed, but the validated runtime contract is currently `Authorization: Bearer ...` rather than `X-API-Key` because of a Mastra issue discovered during live verification. The persistent layout mount, transcript upload path, save-confirmation flow, grounded answer flow, and knowledge-query rendering all still exist and are wired through that temporary bearer bridge. Browser-level end-to-end verification passed on the live web flow.
