...
- [x] **Phase 17: Deployment & Go-Live** - Deploy web to Vercel and agent to Railway (completed 2026-03-05)
...
**Goal**: The platform is accessible at production URLs -- web app on Vercel, agent server on Railway -- with correct environment separation
...
**Success Criteria** (what must be TRUE):
  ...
  3. The agent server is accessible at an HTTPS URL on Railway and responds to health checks
  4. The agent server automatically restarts after a crash (Railway restart policy)
  5. An end-to-end workflow (e.g., Touch 1 pager generation) completes successfully using production URLs -- web on Vercel calls agent on Railway, agent writes to Google Drive via entrypoint credential injection
**Plans**: 1 plan (originally 2; Plan 02 scope absorbed by manual deployment + Railway platform)

Plans:
- [x] 17-01-PLAN.md -- Deployment infrastructure files (Dockerfile, entrypoint.sh, railway.toml)
- ~~17-02-PLAN.md~~ -- Superseded: Vercel and Railway deployments completed manually via platform dashboards
