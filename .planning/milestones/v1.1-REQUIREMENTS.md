# Requirements Archive: v1.1 Infrastructure & Access Control
...
- [x] **DEPLOY-01**: Next.js web app deploys to Vercel with production and preview environments
- [x] **DEPLOY-02**: Production deploys from main branch; preview deploys from other branches
- [x] **DEPLOY-03**: Environment variables configured per Vercel environment (prod Supabase for production, dev Supabase for preview)
- [x] **DEPLOY-04**: Mastra agent server runs on Railway with HTTPS (Docker container via railway.toml)
- [x] **DEPLOY-05**: Agent server auto-restarts on crash (Railway restart policy)
- [x] **DEPLOY-06**: CI/CD: web auto-deploys via Vercel on push; agent auto-deploys via Railway on push
- [x] **DEPLOY-07**: Google Workspace API credentials work in deployed environments (inline JSON via entrypoint script)
...
| Custom domain for agent | Railway provides auto-generated HTTPS URLs; custom domain is future |
...
*Last updated: 2026-03-05 after Phase 17 completion (Oracle VM → Railway)*
